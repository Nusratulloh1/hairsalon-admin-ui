<template>
    <section class="container mx-auto px-5 sm:px-0">
        <Carousel id="gallery" class="w-full" :items-to-show="1" :wrap-around="true" v-model="currentSlide">
            <Slide v-for="(slide, i) in images" :key="i">
                <div class="carousel__item w-full">
                    <img :src="slide" class="w-full h-72 sm:h-[450px] md:h-[766px] object-cover object-center"
                        alt="gallery">
                </div>
            </Slide>
            <template #addons>
                <navigation>
                    <template #next>
                        <span>
                            <svg class=" w-5 h-8 sm:w-[34px] sm:h-[59px]" viewBox="0 0 34 59" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
        ...
        <Carousel id="thumbnails" class="mt-4 sm:mt-8" :items-to-show="4" :wrap-around="true" :breakpoints="breakpoints"
            v-model="currentSlide" ref="carousel">
            <Slide v-for="(slide, i) in images" :key="slide">
                <div class="carousel__item bg-slate-200 w-[95%] h-36 sm:h-[194px]" @click="slideTo(i)">
