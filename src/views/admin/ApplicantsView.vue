  ...
  <div class="flex justify-between mb-4">
    <div class="flex flex-col items-start space-y-2">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 items-center gap-2">
        ...
        <el-select v-model="filter.status" placeholder="Status" size="large" class="w-full">
      ...
      <div class="flex items-center space-x-2">
        <el-button type="success" @click="fetchApplications()">Submit</el-button>
        <el-button type="success" plain @click="resetFilterAndFetch()">Reset</el-button>
      </div>
    </div>
    <div class="flex items-center">
      <el-button type="primary" size="large" :loading="loading" @click="exportToExcel">Export excel</el-button>
      <el-button type="success" size="large" :loading="loading" @click="$router.push({ name: 'create-applicants' })">Add
        Applicant</el-button>
    </div>
  </div>
  <div class="table-wrapper" v-loading="loading">
    <el-table :data="applications.data" stripe style="width: 100%" row-class-name="cursor-pointer">
      <el-table-column prop="first_name" label="Full Name" min-width="220" align="left" fixed show-overflow-tooltip>
        <template #default="{ row }">
          {{ row.first_name }} {{ row.last_name }}
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip prop="email" label="Email" min-width="220" align="left">
        <template #default="{ row }">
          {{ row.creator.email }}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="exam_date" label="Exam Date" min-width="180" align="center" show-overflow-tooltip>
        <template #default="{ row }">
          {{ row.exam_date.name }}
        </template>
      </el-table-column>
      <el-table-column prop="status" label="Status" min-width="100" align="center">
        <template #default="{ row }">
          <el-tag v-if="row.status === 'pending'" type="warning">Pending</el-tag>
          <el-tag v-if="row.status === 'accept'" type="success">Accept</el-tag>
          <el-tag v-if="row.status === 'reject'" type="danger">Reject</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="responder" label="Admin" min-width="180" align="center" show-overflow-tooltip>
        <template #default="{ row }">
      ...
      <el-table-column prop="is_verified" label="Verified" min-width="100" align="center">
        <template #default="{ row }">
      ...
      <el-table-column fixed="right" label="Operations" align="center" min-width="100">
        <template #default="scope">
          <div class="flex justify-center space-x-3">
            <View class="h-4 w-4 text-blue-500" @click="onViewClick(scope.row)" />
          </div>
        </template>
      </el-table-column>
    </el-table>
    <div class="py-4">
      <el-pagination background @current-change="onPaginationChange" layout="prev, pager, next" :page-size="filter.limit"
