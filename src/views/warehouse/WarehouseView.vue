<template>
  <div v-loading="loading" class="space-y-2 md:space-y-8 view">
    <div>
      <h1 class="view__title">{{ $t("app.warehouse") }}</h1>
      ...
      <div class="content__header">
        ...
        <div>
          ...
          <el-button
            type="primary"
            size="large"
            @click="$router.push('warehouse/new-warehouse')"
          >
            <UserPlusIcon class="h-5 w-5 fill-primary mr-2" />
            <span>{{ $t("app.new-warehouse") }}</span>
          </el-button>
        </div>
      </div>
      <div class="content__table py-2 md:py-3.5 px-4 md:px-6">
        <div class="table-wrapper" v-if="warehouses?.data?.length">
          <el-table
          ...
          >
            ...
            >
              <template #default="{ row }">
                {{ $n(row.product_price) }} {{ $t("shared.sum") }}
              </template>
            </el-table-column>
            <el-table-column
              prop="status"
...
import ViewWarehouseDialog from "./components/ViewWarehouseDialog.vue";
...
const currentWarehouse = computed(() => warehouseStore.currentWarehouse);

const onTableRowClick = async (row: any, cell: any) => {
  if (cell.property !== "action") {
    await warehouseStore.getWarehouseById(row.id);
    modal.setTitle(i18n.t("app.warehouse"));
    modal.setWidth("60%");
    modal.showModal();
  }
};
...
const handleActionClick = (command: string, id: string) => {
  if (command === "edit") {
    router.push({ name: "edit-warehouse", params: { id } });
  }
  if (command === "delete") {
    ElMessageBox.confirm(
      ...
      .then(async () => {
        await warehouseStore.removeWarehouse(id);
        ElMessage({
          type: "success",
