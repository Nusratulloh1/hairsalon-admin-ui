<template>
      ...
      <div class="content__header">
        ...
        <div>
          ...
          <el-button
            type="primary"
            size="large"
            @click="$router.push('employees/new-employee')"
          >
            <UserPlusIcon class="h-5 w-5 fill-primary mr-2" />
            <span>{{ $t("app.new-employee") }}</span>
          </el-button>
        </div>
      </div>
      <div class="content__table py-2 md:py-3.5 px-4 md:px-6">
        <div class="table-wrapper" v-if="staffs?.data?.length">
          <el-table
            :data="staffs?.data"
            stripe
            style="width: 100%"
          ...
          >
            ...
            >
              <template #default="{ row }">
                {{ beautify.beautifyCard(row.card_number) }}
              </template>
            </el-table-column>
            <el-table-column
              prop="pay_type"
            ...
            >
              <template #default="{ row }">
                {{ $t(`shared.${beautify.getPayType(row.pay_type)}`) }}
              </template>
            </el-table-column>
            <el-table-column
              prop="work_hour"
            ...
            >
              <template #default="scope">
                <div class="app-light-tag">
                  {{ $t("app.workHour", { value: scope.row.work_hour }) }}
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column width="50" align="right" prop="action">
              <template #default="scope">
                <el-dropdown
                  trigger="click"
                  @command="(command) => handleActionClick(command, scope.row)"
  ...
  <teleport to="#modal" v-if="showModal">
    <ViewEmployeDialog v-if="currentStaff" :staff="currentStaff" />
  </teleport>
</template>
...
import ViewEmployeDialog from "./components/ViewEmployeeDialog.vue";
...
const onTableRowClick = async (row: any, cell: any) => {
  if (cell.property !== "action") {
    try {
      ...
      modal.setTitle(i18n.t("app.employeeCard"));
      modal.setWidth("60%");
      modal.showModal();
    } catch (error) {
...
const handleActionClick = (command: string, row: any) => {
  if (command === "edit") {
    router.push({ name: "edit-employee", params: { id: row.id } });
  }
  if (command === "delete") {
    ElMessageBox.confirm(
      ...
      .then(async () => {
        await staffStore.removeStaff(row.user_id);
        ElMessage({
          type: "success",
