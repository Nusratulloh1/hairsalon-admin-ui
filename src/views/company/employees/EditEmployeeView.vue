...
onMounted(async () => {
  try {
    loading.value = true;
    await staffStore.getStaffById(route.params.id as string);
...
const submitForm = async (ruleForm: any) => {
  try {
    ...
    if (ruleForm.passport_image) {
      const passport_image = await fileStore.uploadFile(
        ruleForm.passport_image as any
      );
      data["passport_image"] = passport_image;
    } else {
      data["passport_image"] = staffStore.currentStaff?.passport_image;
    }

    await staffStore.editStaff(data);
    ElMessage({
